...
I recently undertook a little weekend project to bring all my disparate AWS accounts for various side projects under one AWS Organisation. I originally just wanted centralised billing and SSO, but when I started to think about what I could do with it, the whole thing grew arms and legs like all the best weekend projects. What was supposed to take me a few hours over the weekend has stretched to a job that is at month three and counting. To be clear, I'm doing maybe three hours spread over the course of each week, so I'm not burning the midnight oil or anything. But I've ended up spinning the whole thing into a Landing Zone that provisions accounts under different OUs, manages user permissions out of an SSO directory, and provisions sensible baseline configurations for each account, including automatically creating a Github OIDC role. It's been a good learning experience so far. These are things that are often already in place when I start a new client project, so I don't ever really get to play with them and learn anything about them. 

I've been pleasantly surprised to find that the costs associated with doing all this have been essentially $0 so far. At least, that was true until last month when I took the plunge into configuring Security Hub and Config across the Organisation. I don't think I'm treading new ground by telling anyone that these things cost money, but what really did surprise me was how many rough edges there are on the developer experience for AWS Security Hub and Config that make it tricky to understand where costs are coming from (and how to even get to a 'correct' configuration). Maybe I need to do a bit more Googling, but all the information I could find on this stuff falls under one or more of these categories-

- Hopelessly out of date, so things have changes beyond recognition. 
- Geared towards security engineers who are doing things via ClickOps.
- Targetted towards Organisations that are happy to eat the cost of 'enabling everything everywhere' for Security Hub.

None of those things are what I'm looking for. I'm doing this in 2025, managing everything via Terraform, and I'm not happy spending unnecessary cash just to say I've got security safeguards. I want to figure out what a reasonable security baseline looks like for a personal AWS Organisation. How cheaply can I configure Security Hub whilst still getting a useful amount of information about my Organisation and member accounts?

## Starting Out

Let's do a little thinking about what my environment looks like, and how Security Hub works across the Organisation.
...
1. The Organisation management account is configured with a Resource Control Policy and a Security Hub Policy. The RCP grants our designated Security account access to Org information so it can inspect different OUs and Accounts. It can also attach policies at different points in the hierarchy. The SHP is a newer type of Organisation Policy, and enables Security Hub for specific regions (or all enabled regions). Different OUs can have different SHPs attached, so you could enable Security Hub in your productino OU, but not in your sandbox OU.

2. Member accounts in an Organisation have AWS Config enabled within the member accounts on an account-by-account basis. This is in contrast with Security Hub, where the member accounts have SH enabled from the management account. Normally member accounts in a Landing Zone setup would have some sort of 'baseline' config that can be automatically applied to member accounts. The enabling of AWS Config is an item that needs adding to this basline config. AWS Config reports account resource configs back to the designated security account and stores these reports in an S3 bucket.  It's also worth knowing that Config is a regional service, so must be enabled in every region where you want to ingest security findings.
...
Any chat about Security Hub and Config isn't done until we talk costs. It has a sometimes well-earned reputation for spiking costs unexpectedly and then being hard to find the 'off button' for it all. I will definitely vouch for that being the case! It doesn't help that as of late-2025 AWS is offering a trial period for what is now CSPM. You can get everything set up, and have no idea of what it will cost to run until you're well into implementation. Fine for big orgs, sure, but not fine for little old me. 

Once I realised I was staring down the barrell of a $80 bill for 5 accounts, I went to delete the Terraform code I'd used to configure everything. That's what's so great about Terraform, right? If you want to switch something off, you just delete the resources from the source code and run 'apply', right? Well, sure. Only in this case, deleting the resources from your correctly-enabled central Security Hub config doesn't actually switch everything off across your member accounts. The Config rules don't get deleted from your member accounts, Security Hub stays enabled within them all too. 

To switch everything off, you must explicitly disable Security Hub in your member accounts, and crucially you must also disable it in every _region_ in that member account too! Before you can disable Security Hub in your member account you must first disassociate it from your central management configuration. I knocked together this very simple script to disable everything in my organisation.
...
The key takeaway from all this is that yes, it's easy to blow through your budget with Security Hub. But it's also possible to deploy it in a very focussed way to only the regions you care about. Amazon doesn't make it easy to find out how to do this, but it's very possible with a bit more effort. 

I've also seen a lot of people saying that AWS Config is very expensive. I can't say that's my experience at all. I've conciously set Config to only record resources on a daily basis, rather than continuously. Security Hub findings are still aggregated in real time, but Config recordings happen once per day. This definitely helps to keep charges to a minimum.
...
You might find that in your Organisation these settings aren't quite right for you, but it's worth thinking about whether or not you actually need to flip the 'activate everything' button on services like Security Hub, or whether you could do the fine tuning needed to get just the the benefits you actually need.
